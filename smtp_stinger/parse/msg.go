...
func init() {
	RegisterParser(".msg", &MSGParser{})
}

type msgOleHeader struct {
	Magic [8]byte
	_     [504]byte
}

func (p *MSGParser) Parse(r io.Reader, filePath string, resultsChan chan<- JobResult) error {
	...
	if len(rawBytes) < 512 {
		return errors.New("file size too small for Outlook compound structure")
	}

	var header msgOleHeader
	...
	p.extractMSGTextStreams(rawBytes, filePath, resultsChan)
	return nil
}

func (p *MSGParser) extractMSGTextStreams(data []byte, filePath string, resultsChan chan<- JobResult) {
	i := 512
	n := len(data)
...
func (p *MSGParser) scanAndSubmit(text string, filePath string, resultsChan chan<- JobResult) {
